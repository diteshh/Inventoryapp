  ...
  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: COLORS.navy }}>
      ...
      <View className="px-5 pt-4 pb-3">
        ...
        {/* Stats Bar */}
        <View className="mb-4 flex-row justify-between rounded-2xl p-4" style={{ backgroundColor: COLORS.navyCard, borderWidth: 1, borderColor: COLORS.border }}>
          <View className="items-center">
            <Text style={{ color: COLORS.textSecondary, fontSize: 10, fontWeight: '700' }}>FOLDERS</Text>
            <Text style={{ color: COLORS.white, fontSize: 16, fontWeight: '800' }}>{totalFolders}</Text>
          </View>
          <View className="items-center">
            <Text style={{ color: COLORS.textSecondary, fontSize: 10, fontWeight: '700' }}>ITEMS</Text>
            <Text style={{ color: COLORS.white, fontSize: 16, fontWeight: '800' }}>{totalItems}</Text>
          </View>
          <View className="items-center">
            <Text style={{ color: COLORS.textSecondary, fontSize: 10, fontWeight: '700' }}>TOTAL QTY</Text>
            <Text style={{ color: COLORS.white, fontSize: 16, fontWeight: '800' }}>{totalQuantity}</Text>
          </View>
          <View className="items-center">
            <Text style={{ color: COLORS.textSecondary, fontSize: 10, fontWeight: '700' }}>TOTAL VALUE</Text>
            <Text style={{ color: COLORS.teal, fontSize: 16, fontWeight: '800' }}>{formatCurrency(totalValue)}</Text>
          </View>
        </View>
        ...
        {(currentFolder || breadcrumbs.length > 0) && (
          <View className="mt-3 flex-row items-center flex-wrap gap-1">
            ...
            {currentFolder && (
              <>
                <ChevronRight color={COLORS.textSecondary} size={12} />
                <Text className="text-xs font-medium text-white">{currentFolder.name}</Text>
              </>
            )}
          </View>
        )}
        ...
        <View className="mt-3 flex-row items-center gap-2">
          <TouchableOpacity
            ...
            style={{
              backgroundColor: filterLowStock ? `${COLORS.warning}22` : COLORS.navyCard,
              borderWidth: 1,
          ...
          {(['name', 'date', 'quantity'] as SortBy[]).map((s) => (
            <TouchableOpacity
              ...
              style={{
                backgroundColor: sortBy === s ? `${COLORS.teal}22` : COLORS.navyCard,
                borderWidth: 1,
                borderColor: sortBy === s ? `${COLORS.teal}66` : COLORS.border,
              }}>
              <Text
                className="text-xs font-medium"
                style={{ color: sortBy === s ? COLORS.teal : COLORS.textSecondary }}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            if (viewMode === 'grid') {
              return <GridItem item={item as Item} onPress={() => router.push(`/item/${item.id}`)} />;
            }
            return <ListItem item={item as Item} onPress={() => router.push(`/item/${item.id}`)} />;
          }}
        />
      )}
...
function FolderCard({ folder, onPress }: { folder: EnhancedFolder; onPress: () => void }) {
  return (
    <TouchableOpacity
      onPress={onPress}
      ...
      style={{ backgroundColor: COLORS.navyCard, borderWidth: 1, borderColor: COLORS.border }}>
      {/* Thumbnail Grid */}
      <View className="mr-4 h-[60px] w-[60px] flex-row flex-wrap rounded-xl overflow-hidden bg-navy-light" style={{ backgroundColor: COLORS.navy }}>
        {[0, 1, 2, 3].map((i) => (
          <View key={i} className="h-[30px] w-[30px] items-center justify-center border-[0.5px] border-border" style={{ borderColor: COLORS.border }}>
            {folder.thumbnails?.[i] ? (
      ...
      <View className="flex-1">
        ...
        <Text className="font-bold text-white text-base leading-tight" numberOfLines={1}>{folder.name}</Text>
...
function ListItem({ item, onPress }: { item: Item; onPress: () => void }) {
  ...
  return (
    <TouchableOpacity
      ...
      <View className="flex-1">
        ...
        <Text className="font-bold text-white text-base leading-tight" numberOfLines={1}>{item.name}</Text>
        <View className="mt-1 flex-row items-center">
          <Text style={{ color: COLORS.textSecondary, fontSize: 11 }}>
...
function GridItem({ item, onPress }: { item: Item; onPress: () => void }) {
  ...
  return (
    <TouchableOpacity
      ...
      <View className="p-3">
        <Text className="font-semibold text-white text-sm" numberOfLines={2}>{item.name}</Text>
